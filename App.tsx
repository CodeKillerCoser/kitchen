...
const App: React.FC = () => {
  ...
  const handleSaveSettings = () => {
    ...
    localStorage.setItem("gemini_baseURL", baseURL);
    localStorage.setItem("gemini_selectedModel", selectedModel);
    alert("设置已保存！");
  };
  ...
  const handleFetchModels = async () => {
    ...
    try {
      ...
      if (models.length > 0 && !availableModels.includes(selectedModel)) {
  ...
  useEffect(() => {
    ...
    const savedApiKey = localStorage.getItem("gemini_apiKey");
    if (savedApiKey) setApiKey(savedApiKey);
    const savedBaseURL = localStorage.getItem("gemini_baseURL");
    if (savedBaseURL) setBaseURL(savedBaseURL);
    const savedModel = localStorage.getItem("gemini_selectedModel");
  ...
  useEffect(() => {
    localStorage.setItem(
      "bought_items_v12",
      JSON.stringify(Array.from(boughtItems)),
    );
  }, [boughtItems]);

  const sortedMenu = useMemo(() => {
    if (!plan || !Array.isArray(plan.menu)) return [];
    // 强制周六周日排在最前面
    const order = ["周六", "周日", "周一", "周二", "周三", "周四", "周五"];
    return [...plan.menu].sort((a, b) => {
      const getIdx = (dayStr: string) =>
        order.findIndex((o) => dayStr.includes(o));
  ...
  const handleImageAction = async (
    ...
    setter((prev) => ({
      ...prev,
      [key]: { ...prev[key], isGenerating: true },
    }));
    try {
      const url = await generateStepImage(ownerName, text);
      setter((prev) => ({
        ...prev,
        [key]: { imageUrl: url, isGenerating: false },
      }));
    } catch (e) {
      console.error(e);
  ...
  const recipeIsCompleted = useMemo(() => {
    ...
    return selectedRecipe.steps.every((_, idx) =>
      completedSteps.has(`${selectedRecipe.name}-${idx}`),
    );
    ...
    <div className="min-h-screen pb-32 bg-[#F8FAF8] text-stone-900">
      <header className="bg-white/95 backdrop-blur-xl border-b border-stone-100 sticky top-0 z-40 px-6 py-4 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-black serif-font text-stone-800 tracking-tight">
            岐黄小厨
          </h1>
        </div>
        <div className="flex items-center gap-2">
        ...
        activeTab !== "settings" ? (
          <div className="pt-6 space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="text-center space-y-3 px-4">
              <h2 className="text-4xl font-black text-stone-800 serif-font tracking-tight">
                制定本周计划
              </h2>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <FocusCard
                active={selectedFocus.includes("tasty")}
                onClick={() => toggleFocus("tasty")}
                icon={<Sparkles />}
                label="爆款风味"
                desc="复刻外卖餐厅"
                color="rose"
              />
              <FocusCard
                active={selectedFocus.includes("tcm_authentic")}
                onClick={() => toggleFocus("tcm_authentic")}
                icon={<Leaf />}
                label="正统食养"
                desc="深度药食同源"
                color="emerald"
              />
              <FocusCard
                active={selectedFocus.includes("weight_loss")}
              ...
              />
              <FocusCard
                active={selectedFocus.includes("skin_beauty")}
                onClick={() => toggleFocus("skin_beauty")}
                icon={<HeartIcon />}
                label="养颜滋阴"
                desc="元气女神必备"
                color="pink"
              />
              <FocusCard
                active={selectedFocus.includes("brain_power")}
                onClick={() => toggleFocus("brain_power")}
                icon={<Brain />}
                label="效率续命"
                desc="补脑抗疲劳"
                color="blue"
              />
              <FocusCard
                active={selectedFocus.includes("digestive")}
                onClick={() => toggleFocus("digestive")}
                icon={<Activity />}
                label="健脾祛湿"
                desc="解腻去油去湿"
                color="emerald"
              />
              <FocusCard
                active={selectedFocus.includes("stress_relief")}
                onClick={() => toggleFocus("stress_relief")}
                icon={<Sun />}
                label="疏肝理气"
                desc="疏解工作压力"
                color="amber"
              />
              <FocusCard
                active={selectedFocus.includes("seasonal_health")}
                onClick={() => toggleFocus("seasonal_health")}
                icon={<Thermometer />}
                label="顺时而养"
                desc="时令节气调理"
                color="indigo"
            ...
            <div className="bg-white p-6 rounded-[2.5rem] border border-stone-100 shadow-sm space-y-4">
              ...
              <button
                ...
                className="w-full bg-stone-900 text-white py-6 rounded-[2rem] font-black text-lg shadow-2xl hover:bg-rose-500 active:scale-95 transition-all flex items-center justify-center gap-2"
              >
          ...
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="bg-stone-900 text-white rounded-[2.5rem] p-10 shadow-2xl relative overflow-hidden">
              ...
              <h2 className="text-3xl font-black mb-4 serif-font tracking-tight relative z-10">
                {plan.theme}
              </h2>
              <p className="text-sm text-stone-400 font-medium leading-relaxed italic relative z-10">
                “{plan.philosophy}”
              </p>
            </div>
            ...
            {todayRec && (
              <div className="bg-amber-50 border border-amber-200 rounded-[2.5rem] p-8 shadow-sm flex flex-col sm:flex-row items-center gap-6 animate-in zoom-in-95">
                ...
                <div className="flex-1 text-center sm:text-left">
                  <span className="text-[9px] font-black text-amber-600 bg-white border border-amber-100 px-2 py-0.5 rounded-full uppercase tracking-widest mb-2 inline-block">
                    今日特别推荐
                  </span>
                  <h3 className="text-2xl font-black text-stone-800 serif-font mb-1">
                    {todayRec.name}
                  </h3>
            ...
            <div className="space-y-4">
              {sortedMenu.map((day, dIdx) => (
                <div
                  key={dIdx}
                  className="bg-white rounded-[2rem] overflow-hidden shadow-sm border border-stone-100 transition-all"
                >
                  <div
                    className={`px-6 py-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 border-b border-stone-50 ${day.day.includes("周六") || day.day.includes("周日") ? "bg-amber-50/40" : "bg-stone-50/30"}`}
                  >
                    <div className="flex items-center gap-2 shrink-0">
                      <h3 className="font-black text-stone-800 text-sm tracking-tight">
                        {day.day}
                      </h3>
                      {(day.day.includes("周六") ||
                        day.day.includes("周日")) && (
                        <span className="text-[9px] font-black bg-amber-200 text-amber-800 px-2 py-0.5 rounded-full uppercase">
                          备菜核心
                        </span>
                      )}
                    </div>
                  ...
                  {(day.day.includes("周六") || day.day.includes("周日")) &&
                    day.weekendPrepOperations && (
                              ...
                              <div key={idx} className="space-y-3">
                                <div className="flex gap-4 bg-white p-4 rounded-2xl border border-amber-100 shadow-sm group">
                                  ...
                                  <div className="flex-1 flex justify-between items-start gap-2">
                                    <p className="text-[13px] font-bold text-stone-700 leading-relaxed">
                                      {op}
                                    </p>
                                    <button
                                      onClick={() =>
                                        handleImageAction(day.day, op, "prep")
                                      }
          ...
          <div className="space-y-5 animate-in fade-in slide-in-from-bottom-2">
            <div className="bg-stone-900 text-white p-10 rounded-[2.5rem] shadow-2xl relative overflow-hidden text-center">
              <h3 className="text-2xl font-black serif-font mb-4">
                全周精准清单
              </h3>
              <button
                onClick={copyShoppingList}
                className="bg-white/10 px-6 py-3 rounded-xl hover:bg-white/20 transition-all inline-flex items-center gap-2 mx-auto"
              >
                <Copy className="w-5 h-5" />
                <span className="text-xs font-black uppercase">复制清单</span>
              </button>
            </div>
            {plan.groceryList.map((cat, cIdx) => (
              ...
              >
                <div className="flex items-center justify-between mb-5">
                  <h4 className="text-[11px] font-black text-stone-300 uppercase tracking-widest">
                    {cat.category}
                  </h4>
                  <button
                    onClick={() => copyCategory(cat.category, cat.items)}
                    className="text-[10px] font-black text-rose-500 bg-rose-50 px-3 py-1 rounded-full"
                  >
                    复制分类
                  </button>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {cat.items.map((item, iIdx) => {
                    ...
                    return (
                      <button
                        key={iIdx}
                        onClick={() => toggleBought(cIdx, iIdx)}
                        className={`flex justify-between items-center p-4 rounded-2xl text-[13px] border transition-all ${isChecked ? "bg-stone-50 border-stone-100 text-stone-300" : "bg-white border-stone-100 hover:border-rose-200"}`}
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-5 h-5 rounded-md flex items-center justify-center border ${isChecked ? "bg-emerald-500 border-emerald-500" : "border-stone-200"}`}
                          >
                            {isChecked && (
                          ...
                          <span
                            className={isChecked ? "line-through" : "font-bold"}
                          >
                            {item.name}
                          </span>
                        </div>
          ...
          <div className="space-y-6 pt-6 animate-in fade-in">
            <h2 className="text-3xl font-black text-stone-800 px-4 serif-font">
              心水食谱
            </h2>
            {favorites.length === 0 ? (
              <div className="bg-white rounded-[2rem] p-20 text-center border-2 border-dashed border-stone-100">
                <Star className="w-12 h-12 text-stone-100 mx-auto mb-4" />
                <p className="text-stone-300 font-bold">暂无收藏</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-4">
                {favorites.map((fav, i) => (
                  <div
                    key={i}
                    className="bg-white p-6 rounded-[2rem] border border-stone-100 flex items-center justify-between group cursor-pointer"
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="font-bold text-stone-800">{fav.name}</h4>
                        <p className="text-[10px] text-stone-300 uppercase font-black">
                          {fav.calories}
                        </p>
                      </div>
                    </div>
          ...
          <div className="pt-6 space-y-10 animate-in fade-in duration-500">
            <h2 className="text-3xl font-black text-stone-800 px-4 serif-font">
              设置
            </h2>
            ...
            <div className="bg-white p-6 rounded-[2.5rem] border border-stone-100 shadow-sm space-y-4">
              <div className="flex items-center gap-2 text-[11px] font-black text-stone-300 uppercase tracking-widest px-1">
                <Zap className="w-4 h-4" /> API 配置
              </div>
              <div className="space-y-4">
                <div>
                  <label className="text-sm font-bold text-stone-600 mb-1 block">
                    API Key
                  </label>
                  <input
                    type="password"
                    ...
                    className="w-full bg-stone-50 border border-transparent rounded-2xl p-4 text-sm focus:ring-2 focus:ring-rose-500 outline-none"
                  />
                </div>
                <div>
                  <label className="text-sm font-bold text-stone-600 mb-1 block">
                ...
                <div className="flex items-end gap-2">
                  <div className="flex-grow">
                    ...
                    <select
                      ...
                      className="w-full bg-stone-50 border border-stone-200 rounded-2xl p-4 text-sm focus:ring-2 focus:ring-rose-500 outline-none appearance-none"
                      disabled={availableModels.length === 0}
                    >
                      {availableModels.length === 0 ? (
                        <option>请先加载模型列表</option>
                      ) : (
                  ...
                  <button
                    ...
                    className="shrink-0 bg-stone-100 text-stone-600 px-5 py-4 rounded-2xl font-bold text-sm hover:bg-stone-200 transition-all disabled:opacity-50"
                  >
                    {isModelsLoading ? (
              ...
              <button
                ...
                className="w-full bg-stone-900 text-white py-4 rounded-[2rem] font-black text-lg shadow-xl hover:bg-rose-500 active:scale-95 transition-all flex items-center justify-center gap-2"
              >
                保存设置
              </button>
            </div>
            ...
            <div className="text-center pt-8">
              ...
              <h3 className="text-2xl font-black text-stone-800 serif-font">
                关于岐黄小厨
              </h3>
              <p className="text-stone-400 text-sm font-semibold max-w-sm mx-auto leading-relaxed mt-2">
                中医智慧赋能高效职场，为您量身定制的“极速食养”专家。
              </p>
            </div>
          </div>
        ) : null}
        ...
        <div className="fixed inset-0 bg-stone-950/40 backdrop-blur-md z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 overflow-hidden">
          <div className="bg-[#FEFDFC] w-full max-w-xl sm:rounded-[3rem] h-[96vh] overflow-hidden flex flex-col shadow-2xl animate-in slide-in-from-bottom-full border-t border-stone-100">
            <div className="p-8 border-b border-stone-50 flex justify-between items-start shrink-0">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-[9px] font-black text-rose-500 uppercase tracking-widest bg-rose-50 px-2.5 py-1 rounded-full">
                    {selectedRecipe.calories}
                  </span>
                </div>
                <h3 className="text-3xl font-black text-stone-800 serif-font tracking-tight">
                  {selectedRecipe.name}
                </h3>
                <div className="flex items-center gap-2 mt-3 text-[11px] font-bold text-stone-400">
                  <Leaf className="w-3.5 h-3.5 text-emerald-500" />
                  <span>{selectedRecipe.tcmBenefit}</span>
                </div>
              </div>
              <div className="flex gap-2">
                <button
                  onClick={() => toggleFavorite(selectedRecipe)}
                  className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all shadow-md ${favorites.some((f) => f.name === selectedRecipe.name) ? "bg-rose-500 text-white" : "bg-stone-50 text-stone-300"}`}
                >
                  <HeartIcon
                    className={`w-6 h-6 ${favorites.some((f) => f.name === selectedRecipe.name) ? "fill-current" : ""}`}
                  />
                </button>
                <button
                  onClick={() => setSelectedRecipe(null)}
                  className="w-12 h-12 rounded-2xl bg-stone-50 text-stone-300 flex items-center justify-center"
                >
                  <X className="w-6 h-6" />
                </button>
              </div>
            ...
            <div className="flex-grow overflow-y-auto p-8 space-y-10 pb-40">
              ...
              <section className="space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[9px] font-black text-stone-400 border border-stone-100 px-2 py-0.5 rounded-full uppercase">
                    精准配比
                  </span>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  {selectedRecipe.ingredients.map((ing, idx) => (
                    <div
                      key={idx}
                      className="bg-stone-50/50 border border-stone-100 p-4 rounded-2xl flex flex-col gap-1 hover:border-emerald-200 transition-colors"
                    >
                    ...
                    <div key={idx} className="space-y-5">
                      <div
                        ...
                        onClick={() => {
                          ...
                          if (newDone.has(stepKey)) newDone.delete(stepKey);
                          else newDone.add(stepKey);
                          setCompletedSteps(newDone);
                        }}
                      >
                        <div
                          className={`w-9 h-9 rounded-2xl flex items-center justify-center text-[11px] font-black shrink-0 shadow-md ${isDone ? "bg-emerald-500 text-white" : "bg-stone-900 text-white"}`}
                        >
                          {isDone ? <Check className="w-5 h-5" /> : idx + 1}
                        </div>
                        <div className="flex-1 pt-1.5 flex justify-between items-start">
                          <p
                            className={`text-[15px] leading-relaxed ${isDone ? "text-stone-300 line-through" : "text-stone-800 font-bold"}`}
                          >
                            {step}
                          </p>
                          <button
              ...
              <div className="bg-stone-50 p-8 rounded-[2.5rem] space-y-5">
                <div className="text-center">
                  <h5 className="font-black text-stone-800 text-sm mb-1 tracking-tight">
                    味道合心意吗？
                  </h5>
                ...
                {feedbackSent && (
                  <div className="text-center animate-in fade-in zoom-in">
                    <span className="bg-emerald-500 text-white text-[10px] font-black px-4 py-1.5 rounded-full uppercase">
                      反馈已收到！
                    </span>
                  </div>
                )}
            ...
            <div className="p-8 border-t border-stone-50 bg-white/90 backdrop-blur-xl absolute bottom-0 left-0 right-0 z-10 flex gap-3">
              <button
                onClick={() => setSelectedRecipe(null)}
                className="flex-1 bg-stone-900 text-white py-5 rounded-[2rem] font-black hover:bg-rose-500 transition-all"
              >
                返回列表
              </button>
            </div>
...
const FeedbackBtn: React.FC<{
  ...
  const colors: any = {
    ...
    stone: "text-stone-500 bg-white border-stone-100",
    amber: "text-amber-600 bg-amber-50 border-amber-100",
  };
  return (
    <button
      onClick={onClick}
      className={`flex flex-col items-center gap-2 p-4 rounded-3xl border-2 transition-all ${active ? colors[color] + " scale-110 shadow-lg" : "border-stone-100 bg-white text-stone-300 opacity-60 hover:opacity-100"}`}
    >
      {icon}
...
const FocusCard: React.FC<{
  ...
  return (
    <button
      onClick={onClick}
      className={`p-4 rounded-[2rem] text-left transition-all border-2 ${active ? `${colors[color]} border-transparent shadow-xl scale-[1.02]` : "bg-white border-stone-100 text-stone-800 hover:bg-rose-50/5"}`}
    >
      <div
        className={`w-9 h-9 rounded-xl flex items-center justify-center mb-2.5 ${active ? "bg-white/20" : "bg-stone-50 text-stone-400"}`}
      >
        {icon}
      </div>
      <div className="font-black text-[12px] mb-1 leading-tight">{label}</div>
      <div className="text-[9px] font-bold opacity-70 leading-tight">
        {desc}
      </div>
    </button>
  );
...
const MealRow: React.FC<{
  ...
  <button
    onClick={onClick}
    className="w-full px-6 py-6 text-left hover:bg-rose-50/5 transition-all flex items-center justify-between group"
  >
    <div className="flex items-center gap-5">
      <div
        className={`w-12 h-12 rounded-[1.25rem] flex items-center justify-center font-black text-xs ${type === "午餐" ? "bg-rose-500 text-white" : "bg-stone-800 text-white"}`}
      >
        {type}
      </div>
      <div>
        <div className="flex items-center gap-2 mb-1">
          <span className="text-[9px] font-black text-rose-500 bg-rose-50 px-2 py-0.5 rounded-full">
            {recipe.efficiencyTag}
          </span>
        </div>
        <h4 className="font-bold text-stone-800 group-hover:text-rose-600 transition-colors text-[15px] tracking-tight">
          {recipe.name}
        </h4>
        <div className="flex items-center gap-1.5 mt-1.5 text-[10px] text-stone-300 font-black uppercase tracking-widest">
...
const NavIcon: React.FC<{
  ...
  <button
    onClick={onClick}
    className={`flex items-center gap-2 px-5 py-3 rounded-2xl transition-all ${active ? "bg-white text-stone-950 shadow-xl" : "text-stone-500 hover:text-white hover:bg-white/10"}`}
  >
    {icon}
    <span
      className={`text-xs font-black ${active ? "block" : "hidden sm:block"}`}
    >
      {label}
    </span>
  </button>
